...
class StyleVM: ObservableObject {
    ...
    private func initialiseMobileSDK() {
        ...
        let config = {
            ...
            case .production: return MobileSDKConfig(environment: .production, publicKey: ProjectEnvironment.shared.getPublicKey(), theme: theme)
            case .sandbox: return MobileSDKConfig(environment: .sandbox, publicKey: ProjectEnvironment.shared.getPublicKey(), theme: theme)
            case .staging: return MobileSDKConfig(environment: .staging, publicKey: ProjectEnvironment.shared.getPublicKey(), theme: theme)
            }
        }()
