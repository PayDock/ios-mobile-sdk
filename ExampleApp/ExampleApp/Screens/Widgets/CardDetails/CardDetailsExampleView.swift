...
struct CardDetailsExampleView: View {
    ...
    // `activePrimaryButton: true` (see `ConfigManager.setupCardConfiguration`), so the modifier below
    // is a no-op here — it's written generically to match whichever value the config uses.
            ...
            ScrollViewReader { proxy in
                ScrollView {
                    ...
                    if !viewModel.getConfig().showSubmitButton {
                        ...
                        .accessibilityIdentifier("Tokenise External Button")
