...
protocol WalletService {
    ...
    func initialiseColesPayWalletCharge(
        initializeWalletChargeReq: InitialiseWalletChargeReq) async throws -> WalletData
...
struct WalletServiceImpl: HTTPClient, WalletService {
    ...
    func initialiseWalletCharge(initializeWalletChargeReq: InitialiseWalletChargeReq) async throws -> String {
        let response = try await sendRequest(
            endpoint: WalletEndpoints.initialiseWalletCharge(initialiseWalletChargeReq: initializeWalletChargeReq),
            responseModel: InitialiseWalletChargeRes.self)
        return response.resource.data.token
    }

    func initialiseColesPayWalletCharge(
        initializeWalletChargeReq: InitialiseWalletChargeReq
    ) async throws -> WalletData {
        let response = try await sendRequest(
            endpoint: WalletEndpoints.initialiseColesPayWalletCharge(initialiseWalletChargeReq: initializeWalletChargeReq),
            responseModel: InitialiseWalletChargeRes.self)
    ...
    func createCardToken(tokeniseCardDetailsReq: TokeniseCardDetailsReq) async throws -> String {
        let response = try await sendRequest(
            endpoint: WalletEndpoints.cardToken(tokeniseCardDetailsReq: tokeniseCardDetailsReq),
    ...
    func createIntegrated3DSToken(request: Integrated3DSReq) async throws -> String? {
        let response = try await sendRequest(
            endpoint: WalletEndpoints.integrated3ds(request: request),
            responseModel: Integrated3DSRes.self)
        return response.resource.data.threeDS.token
    }

    func createIntegrated3DSVaultToken(request: Integrated3DSVaultReq) async throws -> Integrated3DSRes {
            let response = try await sendRequest(
                endpoint: WalletEndpoints.integrated3dsVault(request: request),
                responseModel: Integrated3DSRes.self)
            return response
    }
    ...
    func createVaultToken(request: TokeniseCardDetailsReq) async throws -> String {
        let response = try await sendRequest(
            endpoint: WalletEndpoints.vaultToken(request: request),
    ...
    func convertCardTokenToVaultToken(request: ConvertToVaultTokenReq) async throws -> String {
        let response = try await sendRequest(
            endpoint: WalletEndpoints.convertToVaultToken(request: request),
            responseModel: VaultTokenRes.self)
        return response.resource.data.vaultToken
    }
    ...
    func createStandalone3DSToken(request: Standalone3DSReq) async throws -> String? {
        let response = try await sendRequest(
            endpoint: WalletEndpoints.standalone3ds(request: request),
            responseModel: Integrated3DSRes.self)
        return response.resource.data.threeDS.token
    }
    ...
    func captureChargeColesPay(chargeId: String) async throws -> ChargeResponse {
        let response = try await sendRequest(
            endpoint: WalletEndpoints.captureChargeColesPay(chargeId: chargeId),
            responseModel: WalletCaptureRes.self)
        return response.resource.data
    }
}
