...
struct PayPalVaultServiceImpl: HTTPClient, PayPalVaultService {
    ...
    func createSetupTokenData(req: PayPalVaultSetupTokenReq, accessToken: String) async throws -> PayPalVaultSetupTokenRes.SetupTokenData {
        let response = try await sendRequest(
            endpoint: PayPalVaultEndpoints.setupToken(request: req, accessToken: accessToken),
    ...
    func getClientId(gatewayId: String, accessToken: String) async throws -> String {
        let response = try await sendRequest(
            endpoint: PayPalVaultEndpoints.clientId(gatewayId: gatewayId, accessToken: accessToken),
            responseModel: PayPalVaultConfigRes.self)
    ...
    func createPaymentToken(request: PayPalVaultPaymentTokenReq,
                            ...
                            accessToken: String) async throws -> PayPalVaultPaymentTokenRes.PaymentTokenData {
        let response = try await sendRequest(
            endpoint: PayPalVaultEndpoints.paymentToken(setupToken: setupToken, request: request, accessToken: accessToken),
