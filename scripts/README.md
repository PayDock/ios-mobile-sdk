...
Automatic enforcement of branch naming conventions for the Mobile SDK iOS project.
...
- `release/version-or-description` - For release branches (flexible naming)
...
# Ensure validation is working (force setup if needed)
make setup
...
- `scripts/validate-branch-name.sh` - Core validation logic
- `scripts/ensure-validation.sh` - Auto-enforcement and setup
...
```bash
ALLOWED_PREFIXES=("bug/" "task/" "feature/" "release/" "spike/" "hotfix/")
```
