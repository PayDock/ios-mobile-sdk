...
@MainActor
class MPGS3DSTests: XCTestCase {
    var coordinator: MPGS3DSWidget.Coordinator!
    var receivedResult: Result<MPGS3dsResult, MPGS3dsError>?
    ...
    override func setUp() {
        super.setUp()
        receivedResult = nil
        coordinator = MPGS3DSWidget.Coordinator { result in
    ...
    func testValidateToken_withValidToken_callsCompletionWithSuccess() {
        ...
        _ = MPGS3DSWidget(config: ThreeDSConfig(token: validToken)) { result in
    ...
    func testValidateToken_withInvalidToken_callsCompletionWithInvalidTokenError() {
        ...
        _ = MPGS3DSWidget(config: ThreeDSConfig(token: invalidToken)) { result in
            if case let .failure(error) = result {
                switch error {
    ...
    func testValidateToken_withWrongToken_callsCompletionWithInvalidTokenError() {
        ...
        _ = MPGS3DSWidget(config: ThreeDSConfig(token: wrongToken)) { result in
            if case let .failure(error) = result {
                switch error {
                case .invalidToken: expectation.fulfill()
                default: break
